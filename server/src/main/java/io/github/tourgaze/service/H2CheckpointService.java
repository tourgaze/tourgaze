 ...
 *
 * <p>
 * Why: a normal Ctrl-C is already safe — Spring's shutdown hook runs the
 * graceful shutdown and H2 closes the store cleanly. This guards the
 * <em>hard</em>
 * kill instead (taskkill /F, force-closing the console window, power loss),
 * where
 ...
 * metadata. A periodic sync bounds that worst-case loss to the interval and
 * keeps
 * the file from growing unbounded by letting H2 consolidate/reuse old pages.
 *
 * <p>
 * H2-only ({@code @Profile("!postgres")}): {@code CHECKPOINT SYNC} is H2
 * syntax, and the Postgres deployment leaves durability to the server. Pairs
 * with
...
public class H2CheckpointService {
	...
	@Scheduled(fixedDelayString = "${tourgaze.db.checkpoint-ms:60000}", initialDelayString = "${tourgaze.db.checkpoint-ms:60000}")
