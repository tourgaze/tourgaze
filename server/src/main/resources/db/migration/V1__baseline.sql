...
create table activity (avg_hr integer, avg_speed_kmh float(53), distance_km float(53), duration_s integer, elevation_gain_m float(53), elevation_loss_m float(53), calories integer, end_country varchar(2), max_hr integer, max_speed_kmh float(53), avg_cadence integer, max_cadence integer, avg_power_w integer, max_power_w integer, moving_time_s integer, start_country varchar(2), start_lat float(53), start_lon float(53), weather_humidity_pct integer, weather_temp_c float(53), weather_wind_kph float(53), weight_kg float(53), end_time timestamp(6) with time zone, imported_at timestamp(6) with time zone not null, start_time timestamp(6) with time zone, weather_fetched_at timestamp(6) with time zone, source_format varchar(20) not null, gear_id varchar(26), id varchar(26) not null, user_id varchar(26), source_hash varchar(64) not null unique, weather_condition varchar(100), original_filename varchar(500), source_filename varchar(500) not null, activity_type varchar(255), sub_sport varchar(64), description text, end_location varchar(255), name varchar(255), route_geocells text, attributes json, start_location varchar(255), version bigint, updated_at timestamp(6) with time zone, primary key (id));
create table activity_tag (activity_id varchar(26) not null, tag_id varchar(26) not null, primary key (activity_id, tag_id));
create table app_user (date_of_birth date, height_cm integer, max_hr integer, resting_hr integer, ftp_w integer, weight_kg float(53), created_at timestamp(6) with time zone not null, gender varchar(20), id varchar(26) not null, display_name varchar(255), username varchar(255) not null unique, version bigint, updated_at timestamp(6) with time zone, primary key (id));
...
create table gear (assisted boolean not null, weight_kg float(53), created_at timestamp(6) with time zone not null, retired_at timestamp(6) with time zone, id varchar(26) not null, user_id varchar(26), description varchar(255), name varchar(255) not null, type varchar(255), icon varchar(32), version bigint, updated_at timestamp(6) with time zone, primary key (id));
...
create table marker (lat float(53) not null, lon float(53) not null, created_at timestamp(6) with time zone not null, id varchar(26) not null, category varchar(64) not null, description text, label varchar(255) not null, version bigint, updated_at timestamp(6) with time zone, primary key (id));
...
-- User-managed sport / activity-type masterdata. Activities store the sport as a
