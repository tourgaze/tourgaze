...
import { getAllMarkers, createMarker, updateMarker, deleteMarker, type Marker } from '@/api/client'
...
// ── Filter + sort ───────────────────────────────────────────────────────────
const filter = ref('')
const sort = ref<{ key: 'label' | 'category'; dir: 'asc' | 'desc' }>({ key: 'label', dir: 'asc' })
...
const rows = computed(() => {
  ...
  return [...list].sort((a, b) => {
    ...
    if (key === 'category') { av = markerCat(a.category).label; bv = markerCat(b.category).label }
...
function startCreate(lng: number, lat: number) {
  clearDraft()
  editing.value = { id: '', lat, lon: lng, label: '', description: '', category: categories[0].key }
  if (!map) return
  const el = document.createElement('div')
  el.className = 'map-marker-pin'
    ...
    <div class="relative flex-1 min-h-0">
      ...
      <div v-if="editing"
        ...
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold uppercase tracking-wide text-muted-fg">{{ isNew ? 'New marker · drag pin to adjust' : 'Marker' }}</span>
